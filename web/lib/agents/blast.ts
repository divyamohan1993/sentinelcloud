// Blast Radius Calculator (Gap G4).
// BFS over the topology graph from the action target, weighted by criticality.
// Score is 0..100. Anything > 70 is gated to human-in-the-loop.

import type { Action, Topology } from '../types';

const SEVERITY_WEIGHT: Record<string, number> = {
  low: 5, medium: 15, high: 30, critical: 60,
};

const ACTION_BASE: Record<string, number> = {
  rollback: 10,
  restart_pods: 8,
  scale: 4,
  right_size: 6,
  open_pr: 1,
  waf_rule: 5,
  mesh_weight: 7,
  cache_purge: 3,
  feature_flag: 2,
  human_review: 0,
};

export function computeBlastRadius(action: Action, topology: Topology): number {
  const start = topology.nodes.find(n => n.id === action.target || n.name === action.target);
  if (!start) return 25;

  const adj: Record<string, string[]> = {};
  for (const e of topology.edges) {
    (adj[e.from] ??= []).push(e.to);
    (adj[e.to] ??= []).push(e.from);
  }

  const seen = new Set<string>([start.id]);
  const queue: Array<{ id: string; depth: number }> = [{ id: start.id, depth: 0 }];
  let score = ACTION_BASE[action.kind] ?? 5;
  score += SEVERITY_WEIGHT[start.criticality] ?? 10;

  while (queue.length) {
    const { id, depth } = queue.shift()!;
    if (depth > 2) continue;
    const node = topology.nodes.find(n => n.id === id);
    if (!node) continue;
    if (depth > 0) {
      const w = SEVERITY_WEIGHT[node.criticality] ?? 10;
      score += w / (depth + 1);
    }
    for (const next of adj[id] ?? []) {
      if (seen.has(next)) continue;
      seen.add(next);
      queue.push({ id: next, depth: depth + 1 });
    }
  }
  return Math.max(0, Math.min(100, Math.round(score)));
}
